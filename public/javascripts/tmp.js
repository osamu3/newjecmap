//tmp
//kpから緯度経度求める
function tmpGetkp2LatLng() {
	let kp = $('#tmpKp').val();
	let intNmkp = parseInt(kp);
	let lat, lng;

	let floatNmKp = parseFloat(kp); //文字列を浮動小数点数値に変換
	floatNmKp = Math.round(floatNmKp * 10) / 10 // 小数点1桁で四捨五入   ex)123.45→123.5

	for(key in LatLngLst ){
		if (LatLngLst[key].kp == intNmkp) {
			if (LatLngLst[key].route == 9) {
				$('#tmpkp2LatLng').val('lat=' + LatLngLst[key].latlng.lat + ' lng=' + LatLngLst[key].latlng.lng);
			}
		}
	  };

	//$('#tmpkp2LatLng').val('lat=' + LatLngLst[intNmkp].lat + ' lng=' + LatLngLst[intNmkp].lng);//「toFixed」は、70.15 → 70.1  , 70 →70.0にする（小数点第１位まで０(ゼロ)でも表示）
